...
class RetryModifyRequestMiddleware
{
    ...
    private function onFulfilled(RequestInterface $req, array $options)
    {
        return function ($value) use ($req, $options) {
            if (!call_user_func_array($this->decider, array($options['retries'], $req, $value, null))) {
                return $value;
            }

            $req = call_user_func_array (
                $this->requestModifier, array($req, $value)
            );
    ...
    private function onRejected(RequestInterface $req, array $options)
    {
        return function ($reason) use ($req, $options) {
            if (!call_user_func($this->decider, array($options['retries'], $req, null, $reason))) {
                return new RejectedPromise($reason);
            }

            $req = call_user_func_array (
                $this->requestModifier, array($req, null)
            );
