Ext.define 'Toro.view.main.ViewportController',
    ...
    activateView: (hashTag) ->
        ...
        Toro.app.routingToken.current = hashTag

        if lastView
            Toro.app.routingToken.previous = lastView.existingId
        else Toro.app.routingToken.previous = hashTag

        if !existingItem
            newView = Ext.create 'Toro.view.' + (view || 'pages.Error404Window'),
                hideMode: 'offsets'
                existingId: hashTag
