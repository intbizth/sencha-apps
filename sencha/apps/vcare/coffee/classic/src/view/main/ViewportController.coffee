Ext.define 'Vcare.view.main.ViewportController',
    ...
    init: ->
        Vcare.app.on 'LOGIN_SUCCESS', @updateUserView, @
    ...
    activateView: (hashTag) ->
        ...
        Vcare.app.routingToken.current = hashTag

        if lastView
            Vcare.app.routingToken.previous = lastView.existingId
        else Vcare.app.routingToken.previous = hashTag

        if !existingItem
            newView = Ext.create 'Vcare.view.' + (view || 'pages.Error404Window'),
                hideMode: 'offsets'
                existingId: hashTag
