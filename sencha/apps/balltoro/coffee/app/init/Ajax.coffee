Ext.define 'Balltoro.init.Ajax',
    constructor: (app, profile) ->
        IS_DEBUG = Balltoro.Application.IS_DEBUG

        # debug
        Ext.Ajax.on 'beforerequest', (conn, options, eOpts) ->
            # custom path
            options.url = Balltoro.cfg.get('api.baseUrl') + options.path if options.path

            # custom api proxy
            options.url = options.url.replace '/api/', '/' + Balltoro.cfg.get('api.version') + '/'
