Ext.define 'Moboque.model.Base',
    extend: 'Ext.data.Model'

    schema:
        namespace: 'Moboque.model'
    ...
    doRequest: (config) ->
        return Ext.Ajax.request
            ...
            url: config.path || Moboque.cfg.getWith 'api.baseUrl'
...
, (self) ->
    @onExtended (cls, config) ->
        ...
        cls.xhrOverrideOptions = (options) ->
            ...
            # temporary fix because DELETE method show up to POST instead.
            if (['create', 'update'].indexOf(options.action.toLowerCase()) isnt -1) and config.hasFileUpload
                options.params = {}
        ...
        if typeof cls.proxyConfig is 'undefined' and typeof config.api isnt 'undefined'
            url = Moboque.cfg.get 'api.baseUrl'
