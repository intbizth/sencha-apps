Ext.define 'Moboque.view.main.ViewportController',
    ...
    init: ->
        Moboque.app.on 'LOGIN_SUCCESS', @updateUserView, @
    ...
    activateView: (hashTag) ->
        ...
        Moboque.app.routingToken.current = hashTag

        if lastView
            Moboque.app.routingToken.previous = lastView.existingId
        else Moboque.app.routingToken.previous = hashTag

        if !existingItem
            newView = Ext.create 'Moboque.view.' + (view || 'pages.Error404Window'),
                hideMode: 'offsets'
                existingId: hashTag
