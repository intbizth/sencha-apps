Ext.define 'Moboque.view.news_image.Controller',
    extend: 'Moboque.view.base.Controller'
    alias: 'controller.ctrl-news-image'

    createDialog: (record) ->
        vm = @getViewModel()
        title = if !record then 'เพิ่มรายการใหม่' else "แก้ไข #{record.getNewsTitle()}"
        record = vm.prepareData(record)

        options =
            xtype: 'wg-news-image-form'
            title:  title
            viewModel:
                type: 'vm-news-image-form'
                data:
                    record: record
